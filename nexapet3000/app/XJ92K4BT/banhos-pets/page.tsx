...
function BanhosPetsContent() {
  ...
  const [extraServiceEntry, setExtraServiceEntry] = useState<PetBathEntry | null>(null);
  ...
  useEffect(() => {
    const q = query(collection(db, 'produtos'), where('tipo', '==', 'SERVICO'));
    const unsubscribe = onSnapshot(q, (snapshot) => {
  ...
  // Automatic processing for advance payments that were confirmed externally
  useEffect(() => {
    const processPendingCredits = async () => {
      ...
      for (const order of pendingOrdersToProcess) {
        console.log('Processando crédito automático para pedido:', order.id);
  ...
  const handleSaveExtraService = async () => {
    if (!extraServiceEntry || !selectedExtraServiceId) return;

    try {
      const service = availableServices.find(s => s.id === selectedExtraServiceId);
      if (!service) return;

      await addDoc(collection(db, 'historico_banhos'), {
        clienteId: extraServiceEntry.customerId,
        clienteNome: extraServiceEntry.customer.nome,
        petNome: extraServiceEntry.pet.nome,
        data: new Date().toISOString(),
        status: 'Concluído',
        tipo_pacote: 'Serviço Extra',
        servicoExtraNome: service.nome,
        descricao: `Serviço Extra: ${service.nome} para o pet ${extraServiceEntry.pet.nome}`,
        valor: extraServicePrice,
        pago: false,
        faturado: false,
        createdAt: new Date().toISOString()
      });

      showToast(`Serviço ${service.nome} adicionado ao histórico!`, 'success');
      setIsExtraServiceModalOpen(false);
      setExtraServiceEntry(null);
      setSelectedExtraServiceId('');
      setExtraServicePrice(0);
    } catch (err) {
      console.error('Erro ao salvar serviço extra:', err);
      showToast('Erro ao salvar serviço extra.', 'error');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] pb-20">
      ...
      {isExtraServiceModalOpen && extraServiceEntry && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-[32px] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-6 border-b border-gray-100 flex items-center justify-between">
              <h2 className="text-xl font-bold text-gray-900">Adicionar Serviço Extra</h2>
              <button 
                onClick={() => setIsExtraServiceModalOpen(false)}
            ...
            <div className="p-6 space-y-4">
              <div>
                <p className="text-sm font-medium text-gray-500 mb-1">Pet: <span className="text-indigo-600 font-bold">{extraServiceEntry.pet.nome}</span></p>
                <p className="text-sm font-medium text-gray-500 mb-4">Cliente: <span className="text-gray-900 font-bold">{extraServiceEntry.customer.nome}</span></p>
              </div>
              
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-wider text-gray-400">Selecionar Serviço</label>
                <select 
                  ...
                  onChange={(e) => {
                    ...
                    if (service) {
                      setExtraServicePrice(service.precoVenda || 0);
                    }
                  }}
                  className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                >
                  ...
                  {availableServices.map(service => (
                    <option key={service.id} value={service.id}>{service.nome} - R$ {service.precoVenda?.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</option>
                  ))}
                </select>
              </div>
              ...
              {selectedExtraServiceId && (
                <div className="space-y-2 animate-in fade-in slide-in-from-top-2">
                  <label className="text-xs font-bold uppercase tracking-wider text-gray-400">Valor do Serviço (R$)</label>
                  <input 
                    type="number"
                    step="0.01"
                    value={extraServicePrice}
                    onChange={(e) => setExtraServicePrice(Number(e.target.value))}
                    className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-indigo-500 outline-none transition-all font-bold text-indigo-600"
                  />
                  <p className="text-[10px] text-gray-400 italic">* Alterar o valor aqui não afetará o cadastro original do serviço.</p>
                </div>
              )}
              ...
              <div className="pt-4 flex gap-3">
                <button 
                  onClick={() => setIsExtraServiceModalOpen(false)}
                  className="flex-1 px-4 py-3 rounded-2xl font-bold text-gray-500 hover:bg-gray-50 transition-colors"
                >
                  Cancelar
                </button>
                <button 
                  onClick={handleSaveExtraService}
                  disabled={!selectedExtraServiceId}
                  className="flex-1 px-4 py-3 bg-indigo-600 text-white rounded-2xl font-bold hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-100 disabled:opacity-50 disabled:shadow-none"
                >
                  Confirmar
                </button>
              </div>
