...
import { format, addDays, startOfWeek, endOfWeek, eachDayOfInterval, isSameDay, parse, addMinutes, isBefore, startOfDay, isValid } from 'date-fns';
...
function AgendaContent() {
  ...
  useEffect(() => {
    const unsubConfig = onSnapshot(doc(db, 'config', 'empresa'), (docSnap) => {
      if (docSnap.exists()) {
        ...
        setCompanyName(data.nomeEmpresa || '');
        const agendaConfig = data.agendaConfig || {};
        setConfig({
          diasDisponiveis: agendaConfig.diasDisponiveis || ['Segunda', 'Terça', 'Quarta', 'Quinta', 'Sexta', 'Sábado'],
          horarioInicio: agendaConfig.horarioInicio || '08:00',
          horarioFim: agendaConfig.horarioFim || '18:00',
          intervaloMinutos: agendaConfig.intervaloMinutos || 60,
          permitirConflito: !!agendaConfig.permitirConflito
        });
      } else {
        setConfig({
  ...
  const generateTimeSlots = () => {
    if (!config || !config.horarioInicio || !config.horarioFim) return [];
    const slots = [];
    try {
      ...
      const interval = config.intervaloMinutos || 60;
      if (interval <= 0) return [config.horarioInicio];

      let safetyCounter = 0;
      while ((isBefore(current, end) || format(current, 'HH:mm') === config.horarioFim) && safetyCounter < 100) {
  ...
  const filteredAppointments = useMemo(() => {
    if (viewMode === 'daily') {
      const dateStr = isValid(selectedDate) ? format(selectedDate, 'yyyy-MM-dd') : '';
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-4">
              ...
              <h2 className="text-xl font-black text-gray-900 capitalize">
                {isValid(selectedDate) ? format(selectedDate, "EEEE, d 'de' MMMM", { locale: ptBR }) : 'Data Inválida'}
              </h2>
              <button 
                onClick={() => setSelectedDate(addDays(selectedDate, 1))}
                className="p-2 hover:bg-gray-100 rounded-xl transition-all"
                  ...
                  >
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex items-center gap-2 px-3 py-1 bg-gray-50 rounded-xl text-[10px] font-black text-gray-400 uppercase tracking-widest">
                        ...
                        {(() => {
                          const d = parse(app.data, 'yyyy-MM-dd', new Date());
                          return isValid(d) ? format(d, "dd/MM/yyyy") : app.data;
                        })()}
                      </div>
            ...
            <div className="p-8 border-b border-gray-100 flex items-center justify-between bg-indigo-50/50">
              <div>
                ...
                <p className="text-sm font-bold text-indigo-600 uppercase tracking-widest mt-1">
                  {(() => {
                    const d = parse(formData.data, 'yyyy-MM-dd', new Date());
                    return isValid(d) ? format(d, "dd 'de' MMMM", { locale: ptBR }) : formData.data;
                  })()} às {formData.horario}
                </p>
              </div>
              <button 
