...
import { ReceiptWhatsAppModal } from '@/components/ReceiptWhatsAppModal';
...
function PedidosContent() {
  ...
  const [whatsAppReceiptOrder, setWhatsAppReceiptOrder] = useState<OrderData | null>(null);
  ...
  useEffect(() => {
    if (searchParams.get('new_order') === 'true') {
      ...
      router.replace('/XJ92K4BT/pedidos');
    }
  }, [searchParams, router]);
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      setLoading(prev => {
        if (prev) {
          console.warn(`PedidosContent: Timeout de carregamento atingido (${timeoutDuration}ms).`);
          return false;
        }
        return prev;
      });
  ...
  // Listen to History for reconciliation
  useEffect(() => {
    const qHistory = query(collection(db, 'historico_banhos'), where('pedidoId', '!=', ''));
  ...
  // Auto-reconcile paid advance orders
  useEffect(() => {
    if (loading || orders.length === 0) return;

    const reconcile = async () => {
      const pendingReconciliation = orders.filter(order => 
        order.status_pagamento === 'Pago' && 
        order.tipo_cobranca === 'pgto_antecipado' &&
        !order.credito_processado
      );

      for (const order of pendingReconciliation) {
        console.log('Auto-reconciling order:', order.id);
    ...
    <div className="min-h-screen bg-[#F8F9FA] pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        ...
        <OrderList 
          ...
          onReceiptWhatsApp={(order) => setWhatsAppReceiptOrder(order)}
        />
      </div>
      ...
      {whatsAppReceiptOrder && (
        <ReceiptWhatsAppModal
          order={whatsAppReceiptOrder}
          onClose={() => setWhatsAppReceiptOrder(null)}
          onCopy={() => {
