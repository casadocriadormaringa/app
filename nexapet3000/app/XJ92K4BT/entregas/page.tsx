...
function EntregasContent() {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      setLoading(prev => {
        if (prev) {
          console.warn(`EntregasContent: Timeout de carregamento atingido (${timeoutDuration}ms).`);
          return false;
        }
        return prev;
      });
    ...
    try {
      ...
      if (finalPaymentMethod === 'A_PRAZO') {
        ...
        await addDoc(collection(db, 'historico_banhos'), {
          ...
          pago: false,
          faturado: false,
          vendaId: deliveryToFinalize.id,
          dataVencimento: finalDueDate || format(new Date(), 'yyyy-MM-dd'),
