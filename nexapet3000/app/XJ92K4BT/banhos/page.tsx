...
interface CustomerWithStatus extends CustomerData {
...
function BanhosContent() {
  const [loading, setLoading] = useState(true);
  const [customers, setCustomers] = useState<CustomerWithStatus[]>([]);
  const [startDate, setStartDate] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('banhos_startDate') || format(new Date(), 'yyyy-MM-dd');
    }
    return format(new Date(), 'yyyy-MM-dd');
  });
  const [endDate, setEndDate] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('banhos_endDate') || format(new Date(), 'yyyy-MM-dd');
    }
    return format(new Date(), 'yyyy-MM-dd');
  });
  ...
  useEffect(() => {
    localStorage.setItem('banhos_startDate', startDate);
    localStorage.setItem('banhos_endDate', endDate);
  }, [startDate, endDate]);
  ...
  useEffect(() => {
    const fetchBanhos = async () => {
      try {
        ...
        console.error('Erro ao buscar banhos:', err);
      } finally {
        setLoading(false);
      }
    };
    ...
    const timeout = setTimeout(() => {
      setLoading(prev => {
        if (prev) {
          console.warn(`BanhosContent: Timeout de carregamento atingido (${timeoutDuration}ms).`);
          return false;
        }
        return prev;
      });
    ...
    const fetchAuxiliaryData = async () => {
      try {
        const [ordersSnap, bathsSnap, creditsSnap] = await Promise.all([
          ...
          getDocs(query(collection(db, 'historico_banhos'))),
          getDocs(query(collection(db, 'creditos')))
        ]);

        setPendingOrders(ordersSnap.docs.map(doc => ({ id: doc.id, ...doc.data() })));
        setPendingBaths(bathsSnap.docs.map(doc => ({ id: doc.id, ...doc.data() })));
        setPendingCredits(creditsSnap.docs
  ...
  const pendingBalances = useMemo(() => {
    ...
    pendingBaths.forEach(bath => {
      ...
      const isCredit = bath.tipo_pacote?.toLowerCase().includes('crédito') || 
                       bath.tipo_pacote?.toLowerCase().includes('antecipado');
  ...
  const filteredCustomers = useMemo(() => {
    return customers.filter(c => {
      if (!c.proximo_banho) return false;
      return c.proximo_banho >= startDate && c.proximo_banho <= endDate;
    }).sort((a, b) => {
      // Sort by status: em_rota (1), pendente (2), ok (3)
      ...
      // Then by date
      return a.proximo_banho.localeCompare(b.proximo_banho);
    });
  }, [customers, startDate, endDate]);

  const handleUpdateStatus = async (id: string, status: 'pendente' | 'em_rota' | 'ok') => {
    try {
  ...
  const handleSaveOrder = async (data: any) => {
    try {
      if (editingOrder?.id) {
        // Check if status changed to Pago and it's pgto_antecipado
        if (editingOrder.status_pagamento !== 'Pago' && data.status_pagamento === 'Pago' && data.tipo_cobranca === 'pgto_antecipado') {
          await recordAdvancePaymentHistory({ ...editingOrder, ...data });
        }
        ...
        // New order
        if (newData.status_pagamento === 'Pago' && newData.tipo_cobranca === 'pgto_antecipado') {
          await recordAdvancePaymentHistory({ ...newData, id: docRef.id });
        }
  ...
  const handleFinalizeOrCancel = async (customer: CustomerWithStatus, isCancel: boolean = false) => {
    try {
      const customerRef = doc(db, 'clientes', customer.id!);
      const currentDate = parseISO(customer.proximo_banho);
      let nextDate: Date;

      if (customer.tipo_pacote === 'Mensal') {
        nextDate = addDays(currentDate, 7);
      } else if (customer.tipo_pacote === 'Quinzenal') {
        nextDate = addDays(currentDate, 14);
      } else if (customer.tipo_pacote === 'Customizado' && customer.intervalo_customizado) {
        nextDate = addDays(currentDate, customer.intervalo_customizado);
      } else {
        nextDate = addDays(currentDate, 7);
      }
      ...
      const historyData = {
        ...
        data: new Date().toISOString(), // Use launch date instead of scheduled date
        status: isCancel ? 'Cancelado' : 'Concluído',
        tipo_pacote: customer.tipo_pacote,
        valor: isCancel ? 0 : (customer.valor_banho_avulso || 0),
        pago: false, // Always false initially, even if cancelled
        faturado: false,
        createdAt: new Date().toISOString()
      };

      await addDoc(collection(db, 'historico_banhos'), historyData);
      ...
      await addDoc(collection(db, 'todas_movimentacoes'), {
        ...
        detalhes: {
          tipo_pacote: customer.tipo_pacote,
          valor_banho: customer.valor_banho_avulso || 0,
          isCancel: isCancel
        },
        valor: isCancel ? 0 : (customer.valor_banho_avulso || 0),
        createdAt: new Date().toISOString()
      ...
      showToast(isCancel ? 'Banho cancelado e reagendado!' : 'Pacote finalizado e próximo banho agendado!', 'success');
    } catch (err) {
      console.error('Erro ao finalizar/cancelar:', err);
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <div className="text-center">
          <p className="text-gray-600 font-bold">Carregando Clientes de Pacote...</p>
        ...
        <div className="bg-white border-b border-gray-100">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-lg font-bold text-gray-900">Controle de Banhos</h1>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-4">
            {filteredCustomers.length === 0 ? (
              <div className="bg-white p-12 rounded-3xl border border-dashed border-gray-200 text-center">
                ...
                <p className="text-gray-500 font-medium">Nenhum banho agendado para este período.</p>
              </div>
            ) : (
              filteredCustomers.map((customer) => {
                const styles = getStatusStyles(customer.status_banho);
                return (
                  <div 
                    key={customer.id} 
                  ...
                  <div className="flex-1 text-center md:text-left">
                    <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 mb-1">
                      ...
                      <h3 className="text-lg font-bold text-gray-900">{customer.nome}</h3>
                      {isVencido(customer.proximo_banho) && (
                    ...
                    <p className="text-sm text-gray-500 mb-1">
                      Próximo Banho: <span className="font-semibold text-indigo-600">{format(parseISO(customer.proximo_banho), 'dd/MM/yyyy')}</span>
                    </p>
                    {customer.data_vencimento && (
                      <p className="text-sm text-gray-500 mb-2">
                        Próximo Vencimento: <span className="font-semibold text-amber-600">{format(parseISO(customer.data_vencimento), 'dd/MM/yyyy')}</span>
                      </p>
                    )}
                    <div className="flex flex-wrap justify-center md:justify-start gap-2">
                      ...
                      {pendingBalances[customer.id!] !== undefined && (
                        <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-lg flex items-center gap-1 ${
                          ...
                          {pendingBalances[customer.id!] >= 0 
                            ? 'Pendente' 
                            : 'Crédito'}: R$ {Math.abs(pendingBalances[customer.id!]).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap justify-center gap-2">
                    ...
                    <button 
                      onClick={() => handleUpdateStatus(customer.id!, 'ok')}
                      className="p-3 bg-green-50 text-green-600 rounded-2xl hover:bg-green-100 transition-colors"
                    ...
                    <button 
                      onClick={() => handleUpdateStatus(customer.id!, 'em_rota')}
                    ...
                    <button 
                      onClick={() => handleUpdateStatus(customer.id!, 'pendente')}
                    ...
                    <button 
                      onClick={() => handleFinalizeOrCancel(customer)}
                    ...
                    <button 
                      onClick={() => handleFinalizeOrCancel(customer, true)}
